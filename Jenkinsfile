...
def checkWorkflowJobStatus(String workflowKey, String titlePrefix) {
    ddef jobNameMap = [
        "AddTag": "AddTag（新增 Tag）",
        "AddThirdLevelRandom": "AddThirdLevelRandom（設定三級亂數）",
        "AttachAntiBlockTarget": "AttachAntiBlockTarget（新增抗封鎖目標）",
    ...
    def envName = "測試環境"
    if (BASE_URL.contains("vir999.com")) envName = "DEV"
    else if (BASE_URL.contains("staging168.com")) envName = "STAGING"
    else if (BASE_URL.contains("vir000.com")) envName = "PROD"

    catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
        def exported = readJSON file: '/tmp/exported_env.json'
        def workflowId = exported.values.find { it.key == workflowKey }?.value

        if (!workflowId) {
            error("❌ 無法從 /tmp/exported_env.json 中取得 ${workflowKey}")
        }
            ...
            if (failedJobs || blockedJobs) {
                ...
                def message = """{
                    "cards": [
                        {
                        "header": {
                            "title": "🚨 Jenkins - ${titlePrefix} Job狀態檢查 - 異常",
                            "subtitle": "Workflow ID: ${workflowId}",
                            "imageUrl": "https://uxwing.com/wp-content/themes/uxwing/download/brands-and-social-media/postman-icon.png",
                            "imageStyle": "AVATAR"
                        },
        ...
        if (!success) {
            ...
            def timeoutMsg = """{
                "cards": [{
                    "header": {
                        "title": "⏰ ${titlePrefix} Job狀態檢查輪詢超時失敗",
                        "subtitle": "Workflow Timeout",
                        "imageUrl": "https://uxwing.com/wp-content/themes/uxwing/download/brands-and-social-media/postman-icon.png"
                    },
                    "sections": [{
            ...
            writeFile file: 'payload.json', text: timeoutMsg
            withEnv(["WEBHOOK_URL=${WEBHOOK_URL}"]) {
                sh 'curl -k -X POST -H "Content-Type: application/json" -d @payload.json "$WEBHOOK_URL"'
            }
